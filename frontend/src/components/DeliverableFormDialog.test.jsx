...
async function fillRequiredFieldsForCreate() {
  ...
  await pickDate(/due date/i, '2027-06-01')
}
...
describe('create mode', () => {
  it('renders the create title, NOT_STARTED default, and disabled submit', async () => {
    renderDialog({ open: true, deliverable: null, onClose: vi.fn(), onSaved: vi.fn() })
  ...
  it('submits the expected payload with owner_id/completed_at defaulted to null', async () => {
    ...
    renderDialog({ open: true, deliverable: null, onClose: vi.fn(), onSaved })
    await fillRequiredFieldsForCreate()
  ...
  it('sets an optional Completed On date via the date picker', async () => {
    mockCreateDeliverable.mockResolvedValue({ id: 'd1' })
    renderDialog({ open: true, deliverable: null, onClose: vi.fn(), onSaved: vi.fn() })
    await fillRequiredFieldsForCreate()
    await pickDate(/completed on/i, '2027-06-15')
    await userEvent.click(screen.getByRole('button', { name: 'Create Deliverable' }))

    await waitFor(() =>
      expect(mockCreateDeliverable).toHaveBeenCalledWith(expect.objectContaining({ completed_at: '2027-06-15' })),
    )
  })
})
...
describe('edit mode', () => {
  ...
  it('renders the edit title with the deliverable name and pre-fills fields', async () => {
    renderDialog({ open: true, deliverable: existingDeliverable, onClose: vi.fn(), onSaved: vi.fn() })
    ...
    expect(within(dueDateGroup).getByRole('spinbutton', { name: 'Year' })).toHaveTextContent('2027')
    expect(within(dueDateGroup).getByRole('spinbutton', { name: 'Month' })).toHaveTextContent('06')
    expect(within(dueDateGroup).getByRole('spinbutton', { name: 'Day' })).toHaveTextContent('01')
  ...
  it('submits an update via updateDeliverable', async () => {
    ...
    renderDialog({ open: true, deliverable: existingDeliverable, onClose: vi.fn(), onSaved })
    await waitFor(() => expect(mockListProjects).toHaveBeenCalled())
    ...
    await waitFor(() =>
      expect(mockUpdateDeliverable).toHaveBeenCalledWith(
        'd1',
        expect.objectContaining({ name: 'Design Doc', status: 'COMPLETED', due_date: '2027-06-01' }),
      ),
    )
